...
import { Camera, Mic, MicOff, Phone, ShieldCheck, Volume2 } from 'lucide-react'
...
export default function InterviewClient({ role, accessCode }: Props) {
  ...
  function speak(text: string) {
    ...
    const voice = voices.find(item => /Microsoft (Jenny|Aria|Sonia)|Google UK English Female|Google US English|Samantha|Karen|Moira|Zira/i.test(item.name)) || voices.find(item => /^en(-|_)/i.test(item.lang) && /female|natural|neural|online/i.test(`${item.name} ${item.voiceURI}`)) || voices.find(item => /^en(-|_)/i.test(item.lang))
