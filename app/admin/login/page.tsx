...
export default function AdminLogin() {
  ...
  async function login(e:React.FormEvent) { e.preventDefault(); setError(''); if (!email.toLowerCase().endsWith('@care-international.org')) { setError('Use your CARE International staff email.'); return } const {error} = await supabase.auth.signInWithPassword({email,password}); if (error) { setError('Invalid email or password.'); return } router.push('/admin') }
