...
export default async function AdminPage() {
  ...
  const { data: campaigns } = await supabase.from('email_campaigns').select('id,subject,recipient_count,sent_count,failed_count,status,created_at').order('created_at', { ascending:false }).limit(20)
