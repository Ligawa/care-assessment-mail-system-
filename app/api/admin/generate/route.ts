...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!result.output || result.output.questions.length !== input.question_count || result.output.salary_min > result.output.salary_max) return NextResponse.json({ error: 'Gemini returned an incomplete assessment draft.' }, { status: 502 })
