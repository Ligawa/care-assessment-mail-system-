...
export async function POST(request: Request) {
  ...
  const { error: decisionUpdateError } = await supabase.from('assessment_submissions').update({ decision_status: parsed.data.decision, decision_at: new Date().toISOString(), notification_status: 'sending', notification_error: null }).eq('id', candidate.id)
  if (decisionUpdateError) return NextResponse.json({ error: `Unable to save candidate decision: ${decisionUpdateError.message}` }, { status: 500 })
  ...
  try {
    ...
    const { error: sentUpdateError } = await supabase.from('assessment_submissions').update({ notification_status: 'sent', notification_error: null, notified_at: new Date().toISOString() }).eq('id', candidate.id)
    if (sentUpdateError) return NextResponse.json({ error: `Email sent, but status could not be saved: ${sentUpdateError.message}` }, { status: 500 })
