...
export default async function InterviewPage({ params }: { params: Promise<{ accessCode: string }> }) {
  ...
  const { data: interview } = await supabase.from('interview_sessions').select('role,access_code,status').eq('access_code', accessCode.toUpperCase()).maybeSingle()
  const role = assessment?.title || interview?.role
  if (!role || (interview && interview.status === 'completed')) notFound()
  const { data: voice } = await supabase.from('interview_settings').select('voice_uri,voice_name,voice_lang').eq('id', 1).maybeSingle()
  return <InterviewClient role={role} accessCode={assessment?.access_code || interview?.access_code || accessCode.toUpperCase()} voiceUri={voice?.voice_uri || ''} />
}
